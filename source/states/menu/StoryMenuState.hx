...
class StoryMenuState extends MusicBeatState
{
	...
	var grpBacks:FlxTypedGroup<FlxSprite>;
	...
	override function create()
	{
		...
		grpBacks = new FlxTypedGroup<FlxSprite>();
		add(grpBacks);
		...
		weekScoreTxt = new FlxText(8, 10, 0,"");
		weekScoreTxt.setFormat(Main.gFont, 36, 0xFFFFFFFF, LEFT);
		add(weekScoreTxt);
		
		weekNameTxt = new FlxText(8, 10, 0,"");
		...
		trackTxt.setFormat(Main.gFont, 36, 0xFFFFFFFF, CENTER);
	...
	override function update(elapsed:Float)
	{
		...
		if(canSelect)
		{
			...
			if(Controls.justPressed(ACCEPT))
			{
				...
				if(weekList[curWeek-1] != null) {
					if(weekList[curWeek-1].freeplayUnlock != null)
						lockedWeek = !SongData.savedWeeks.get(weekList[curWeek-1].weekFile);
				}
				if(!lockedWeek) {
					canSelect = false;
					...
					weekSpr._update = function(elapsed:Float)
					{
						...
						{
							colorCount = 0;
							if(weekSpr.color == 0xFFFFF700)
								weekSpr.color = 0xFFFFFFFF;
							else
								weekSpr.color = 0xFFFFF700;
						}
					}
	...
	public function changeWeek(change:Int = 0)
	{
		...
		{
			week.alpha = 0.4;
			if(week.ID == curWeek && !lockedWeek)
				week.alpha = 1;
		}

		for(back in grpBacks.members) {
			back.alpha = 0;
			if(back.ID == curWeek)
				back.alpha = 1;
		}
